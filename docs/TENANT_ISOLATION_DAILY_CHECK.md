...
- **Optional webhook:** if `ADMIN_DASHBOARD_NOTIFY_URL` and `CORE_UPDATE_NOTIFY_SECRET` are set (same as core-update operator), the workflow POSTs a `tenant_isolation_failed` event on failure.
